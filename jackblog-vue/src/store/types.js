export const ADD_TO_CART = 'ADD_TO_CART'
export const CHECKOUT_REQUEST = 'CHECKOUT_REQUEST'
export const CHECKOUT_SUCCESS = 'CHECKOUT_SUCCESS'
export const CHECKOUT_FAILURE = 'CHECKOUT_FAILURE'
export const RECEIVE_PRODUCTS = 'RECEIVE_PRODUCTS'

export const LOGIN_SUCCESS = 'LOGIN_SUCCESS'
export const USERINFO_SUCCESS = 'USERINFO_SUCCESS'
export const USERINFO_FAILURE = 'USERINFO_FAILURE'
export const LOGOUT_USER = 'LOGOUT_USER'
export const UPDATE_USER_SUCCESS = 'UPDATE_USER_SUCCESS'

export const CHANGE_STYLE_MODE = 'CHANGE_STYLE_MODE'
export const GET_INDEX_IMG = 'GET_INDEX_IMG'
export const GET_CAPTCHAURL = 'GET_CAPTCHAURL'

export const SUCCESS_GET_SNSLOGINS = 'SUCCESS_GET_SNSLOGINS'
export const FAILURE_GET_SNSLOGINS = 'FAILURE_GET_SNSLOGINS'

export const GET_TAG_LIST_SUCCESS = 'GET_TAG_LIST_SUCCESS'
export const GET_TAG_LIST_FAILURE = 'GET_TAG_LIST_FAILURE'
export const CHANGE_OPTIONS = 'CHANGE_OPTIONS'

export const ARTICLE_LIST = 'ARTICLE_LIST'
export const ARTICLE_DETAIL = 'ARTICLE_DETAIL'
export const TOGGLE_LIKE = 'TOGGLE_LIKE'
export const PRENEXT_ARTICLE = 'PRENEXT_ARTICLE'
export const ADD_ARTICLE_LIST = 'ADD_ARTICLE_LIST'
export const REQUEST_ARTICLE_LIST = 'REQUEST_ARTICLE_LIST'
export const GET_ARTICLE_LIST_FAILURE = 'GET_ARTICLE_LIST_FAILURE'
export const ARTICLE_TOTAL = 'ARTICLE_TOTAL'

export const COMMENT_LIST = 'COMMENT_LIST'
export const SUCCESS_ADD_COMMENT = 'SUCCESS_ADD_COMMENT'
export const SUCCESS_ADD_REPLY = 'SUCCESS_ADD_REPLY'
export const GET_COMMENT_LIST_FAILURE = 'GET_COMMENT_LIST_FAILURE'
export const SUCCESS_GET_APPS = 'SUCCESS_GET_APPS'
export const FAILURE_GET_APPS = 'FAILURE_GET_APPS'

export const SHOW_MSG = 'SHOW_MSG'
export const HIDE_MSG = 'HIDE_MSG'

export const GET_ACCESS_TOKEN = 'GET_ACCESS_TOKEN'
export const REFRESH_ACCESS_TOKEN = 'REFRESH_ACCESS_TOKEN'

export const EDITION_LIST = 'EDITION_LIST'
export const FAILURE_GET_EDITION = 'FAILURE_GET_EDITION'
export const CURRENT_EDITION = 'CURRENT_EDITION'
export const CURRENT_ARTICLE = 'CURRENT_ARTICLE'

export const GET_ARTICLE_ORIGIN = 'GET_ARTICLE_ORIGIN'
export const GET_ARTICLE_ORIGIN_LOCAL = 'GET_ARTICLE_ORIGIN_LOCAL'
export const FAILURE_GET_ARTICLE_ORIGIN = 'FAILURE_GET_ARTICLE_ORIGIN'
export const UPDATE_ARTICLE = 'UPDATE_ARTICLE'
export const SAVE_ARTICLE = 'SAVE_ARTICLE'
export const SAVE_EDITION = 'SAVE_EDITION'
export const ADD_ARTICLE = 'ADD_ARTICLE'
export const ADD_EDITION = 'ADD_EDITION'
export const ARTICLE_SUCCESS = 'ARTICLE_SUCCESS'
export const EDITION_SUCCESS = 'EDITION_SUCCESS'
export const ARTICLE_FAILURE = 'ARTICLE_FAILURE'
export const EDITION_FAILURE = 'EDITION_FAILURE'
export const DELETE_ARTICLE = 'DELETE_ARTICLE'
export const DELETE_EDITION = 'DELETE_EDITION'
export const MOVE_ARTICLE = 'MOVE_ARTICLE'

export const CHANGE_SPLITER = 'CHANGE_SPLITER'

export const GET_CATEGORY = 'GET_CATEGORY'
export const GET_CATEGORY_ARTICLE = 'GET_CATEGORY_ARTICLE'
export const GET_CATEGORY_ARTICLE_FAILURE = 'GET_CATEGORY_ARTICLE_FAILURE'

export const GET_ARTICLE_MANA = 'GET_ARTICLE_MANA'
export const GET_ARTICLE_MANA_FAILURE = 'GET_ARTICLE_MANA_FAILURE'

export const CHANGE_EDIT_STATUS ='CHANGE_EDIT_STATUS'
